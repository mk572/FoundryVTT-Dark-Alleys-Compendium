...
Rogue's mechanical text now carries real archmage `[[...]]`/`@`-syntax
instead of plain English (13a-rules-db's `export-to-foundry.mjs` now
reads from that project's `dist/tagged/` output, generated by its
tagger, instead of `data/` directly — see that project's `docs/
TASKS-DONE.md`'s "T003: tagger wired into the export pipeline" entry
for the full pipeline change). 9 of Rogue's entries use a low-confidence
default `@atk` slot guess (arcane, `'a'`) since Rogue has no confirmed
entry in the tagger's own `CLASS_ATK_SLOT` table — flagged, not
silently guessed; see that project's `dist/tagged/REVIEW-rogue.md`.
**Every other class in this module still ships plain-English mechanical
text** — only Rogue has gone through the new tagged-export path so far.
